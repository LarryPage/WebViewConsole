...
- (void)showPlaceHolder:(BOOL)show
{
    if (show) {
        ...
        CGRect rect = [placeHolderLabel.text boundingRectWithSize:CGSizeMake(placeHolderLabel.wbtWidth, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:placeHolderLabel.font} context:nil];
        CGSize size=CGSizeMake(ceilf(rect.size.width), ceilf(rect.size.height));
        placeHolderLabel.wbtHeight = size.height;
    } else {
        placeHolderLabel.hidden = YES;
    }
}
...
- (void)setMaxNumberOfLines:(NSInteger)n
{
    ...
    if (WBAvalibleOS(7)) {
        ...
        CGRect rect = [newText boundingRectWithSize:CGSizeMake(100, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.font} context:nil];
        CGSize size=CGSizeMake(ceilf(rect.size.width), ceilf(rect.size.height));
